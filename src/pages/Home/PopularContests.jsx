            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {
                    popular.map((contest, index) => (
                        <Link
                            ...
                            key={contest._id}
                            className="group relative bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-primary/10 hover:border-primary dark:border-gray-700 flex flex-col h-full w-full"
                            style={{
                                animationDelay: `${index * 100}ms`
                            }}
                        >
                            {/* Image Section */}
                            <figure className="h-40 overflow-hidden relative shrink-0">
                                <img
                                    src={contest.image}
                                    alt={contest.name}
                                    loading="lazy"
                                ...
                                <div className="absolute top-3 right-3 z-10">
                                    <span className="bg-white/95 text-primary px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded shadow-sm">
                                        {contest.type}
                                    </span>
                                </div>
                                ...
                                <div className="absolute bottom-3 left-3 right-3 z-10">
                                    <h2 className="text-lg font-bold text-white mb-0.5 line-clamp-1 drop-shadow-md group-hover:text-secondary transition-colors">
                                        {contest.name}
                                    </h2>
                            ...
                            <div className="p-4 flex flex-col flex-grow">
                                <div className="flex justify-between items-center mb-3 pb-3 border-b border-gray-100 dark:border-gray-700">
                                    <div className="flex flex-col">
                                        <span className="text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-wide font-bold">Prize</span>
                                        <span className="text-base font-bold text-gray-900 dark:text-white flex items-center gap-1">
                                            <FaTrophy className="text-accent text-sm" /> ${contest.prize}
                                        </span>
                                    </div>
                                    <div className="text-right flex flex-col items-end">
                                        <span className="text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-wide font-bold">Ends In</span>
                                        <span className={`text-xs font-bold ${formatDeadline(contest.deadline) === 'Ended' ? 'text-red-500' : 'text-green-600'}`}>
                                ...
                                <div className="mt-auto">
                                    <button className="w-full btn btn-primary btn-sm min-h-[2rem] h-8 rounded text-white font-bold text-xs shadow-sm hover:shadow group-hover:scale-[1.02] transition-transform">
